...
public class GeneralTopologyContext implements JSONAware {
    ...
    private Map<Integer, String> _taskToComponent;                          // task-id->component-id
    private Map<String, List<Integer>> _componentToTasks;                   // component-id->task-ids
    private Map<String, Map<String, Fields>> _componentToStreamToFields;    // component-id->stream-id->fields
    ...
    public Map<String, Map<String, Grouping>> getTargets(String componentId) {
        ...
        // topology所有的component-id. 定义在StormTopology的map<string, SpoutSpec>,其中的key就是component-id
        for(String otherComponentId: getComponentIds()) {
            // StormTopology的每个component对象都有ComponentCommon引用, 而ComponentCommon有inputs Map.
            Map<GlobalStreamId, Grouping> inputs = getComponentCommon(otherComponentId).get_inputs();
            // GlobalStreamId包括component-id和stream-id
            for(GlobalStreamId id: inputs.keySet()) {
                if(id.get_componentId().equals(componentId)) {
                    ...
                    // stream-id, {component-id, Grouping}
