...
public class Cluster {
    ...
    public boolean needsScheduling(TopologyDetails topology) {
        int desiredNumWorkers = topology.getNumWorkers();   // 配置的worker数
        int assignedNumWorkers = this.getAssignedNumWorkers(topology);// 已经分配的worker

        if (desiredNumWorkers > assignedNumWorkers) { // 已经分配的<配置的,需要调度
            return true;
        }
        // 如果已经分配的>=配置的了, 则查看是否还有未分配的executors.如果有则需要调度. 没有就不需要
    ...
    public Map<ExecutorDetails, String> getNeedsSchedulingExecutorToComponents(TopologyDetails topology) {
        ...
        // allExecutors = topology's all executors - toplogy's assignment's executors = 未分配的executors
        // 返回未分配的　executor -> component
    ...
    public Collection<ExecutorDetails> getUnassignedExecutors(TopologyDetails topology) {
        ...
        if (assignment != null) {
            // 获取调度任务的所有executors: 这些executors已经被分配到slots上了
            Set<ExecutorDetails> assignedExecutors = assignment.getExecutors();
            // 除了已经分配的, 剩余的就是还没有分配任务的executors
    ...
    public boolean isSlotOccupied(WorkerSlot slot) {
        for (SchedulerAssignment assignment : this.assignments.values()) {
            // assignment.executorToSlot.containsValue(slot)
            if (assignment.isSlotOccupied(slot)) {
                return true;
            }
        }
