...
(defnk add-supervisor [cluster-map :ports 2 :conf {} :id nil]
  (let [tmp-dir (local-temp-path)
    ...
    (swap! (:tmp-dirs cluster-map) conj tmp-dir)   ;; 更新(swap!)cluster-map中:supervisor的值
    daemon      ;; dameon位body的返回值即mk-supervisor返回的supervisor对象
    ))
...
(defnk mk-local-storm-cluster [:supervisors 2 :ports-per-supervisor 3 :daemon-conf {} :inimbus nil :supervisor-slot-port-min 1024]
  (let [zk-tmp (local-temp-path)
        ...
        supervisor-confs (if (sequential? supervisors)  ;; list和vector都是sequential. 而只有list是seq  http://clojuredocs.org/clojure_core/clojure.core/sequential_q
                           supervisors
...
(defn submit-local-topology [nimbus storm-name conf topology]
  ;; 验证配置有效性, 和StormSubmmiter.submitTopology版本的差不多一样的流程
