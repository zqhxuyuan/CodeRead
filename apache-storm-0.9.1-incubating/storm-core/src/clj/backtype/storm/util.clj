...
;; 分隔路径. zookeeper的节点都是以/开头的绝对路径
;; 返回一个向量, 包含了某个节点的所有路径的部分
;; path=/storm/storms/topo-id, 则返回值为: #{storm storms topo-id}
...
;; path=/storm/storms/topo-id, 则父路径为: /storm/storms
...
;; 比如toks=#{storm storms topo-id}, 则返回值为/storm/storms/topo-id
(defn toks->path [toks]
  (str "/" (str/join "/" toks))
  )
...
;; 接受一个aMap,其中aMap的value是aFun的参数. 最终返回aMap的key和aFun(value)组成的一个新Map
...
;; parent=/storm/storms name=topo-id
...
(defn read-dir-contents [dir]
  (if (exists-file? dir)
    (let [content-files (.listFiles (File. dir))]
      (map #(.getName ^File %) content-files))  ; 存在文件,返回文件名列表
    [] )) ; 如果dir下不存在文件,则返回[]
