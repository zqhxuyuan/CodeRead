...
(defn get-alive-assigned-node+port->executors [cluster topology-id]
  (let [existing-assignment (.getAssignmentById cluster topology-id)  ; SchedulerAssignment
        ...
        executor->node+port (into {} (for [[^ExecutorDetails executor ^WorkerSlot slot] executor->slot
                                       ...
                                       {executor node+port}))
        alive-assigned (reverse-map executor->node+port)] ; 反转: node+port->executor
    alive-assigned))

(defn- schedule-topology [^TopologyDetails topology ^Cluster cluster]
  (let [topology-id (.getId topology)
        ;被free的slots会出现在available-slots里.
        ...
        all-executors (->> topology
                          ...
                          set)
        ;必须重新计算, 因为刚刚free的slot assignment关系. 被free后,不会出现在alive里,因为从executor->slot中移除
        ;alive里面保存的是正常分配的.
        ...
        ;上一步已经把bad slot都free, 仍然alive说明该slot不需要被reassign
        ;可以重新分配: 之前分配的不合理,需要重新分配; 之前就没有分配过,某个slot之前没有executor,它就是可用的
        reassign-slots (take (- total-slots-to-use (count alive-assigned))
                             (sort-slots available-slots))
        ;all-executors - alive-executors, 就是需要被重新分配的executors
