...
;; existing-slots = alive-assigned = node+port->executors
(defn- bad-slots [existing-slots num-executors num-workers]
  (if (= 0 num-workers)
    '()
    (let [distribution (atom (integer-divided num-executors num-workers))
          keepers (atom {})]
      (doseq [[node+port executor-list] existing-slots
              :let [executor-count (count executor-list)]]  ; node+port上的executors数量
      ...
      (filter
        (fn [[node port]]
          (if-not (.isBlackListed cluster node)
            (if-let [supervisor (.getSupervisorById cluster node)]  ; node = supervisor-id
              (.contains (.getAllPorts supervisor) (int port))      ; assigned slots is on node's ports
              ))))))
...
(defn default-schedule [^Topologies topologies ^Cluster cluster]
  ; 需要调度的topologies=List<TopologyDetails>. Topologies保存的是集群中所有拓扑的{topology-id TopologyDetails}
  (let [needs-scheduling-topologies (.needsSchedulingTopologies cluster topologies)]
    (doseq [^TopologyDetails topology needs-scheduling-topologies
            :let [topology-id (.getId topology)
                  available-slots (->> (.getAvailableSlots cluster) ; List<WorkerSlot>
                                       (map #(vector (.getNodeId %) (.getPort %)))) ; #(nodeId port)
                  ...
                  total-slots-to-use (min (.getNumWorkers topology)
                                          (+ (count can-reassign-slots) (count available-slots))) ; 这两部分没有交集,可以+
...
;; 什么时候调用该方法: 在mk-assignments的第三步compute-new-topology->executor->node+port生成cluster后调用
